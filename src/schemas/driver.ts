...
export const DriverAdminFieldsSchema = z.object({
  ...
  // Permite isentar o motorista de desconto por X semanas (ex: promoção inicial)
  adminFeeExemption: z.object({
    isExempt: z.boolean().default(false),                   // Se está em período de isenção
    exemptionStartDate: z.string().nullable().optional(),  // Data início (ISO string)
    exemptionWeeks: z.number().min(0).default(0),          // Quantas semanas de isenção
    createdAt: z.string().nullable().optional(),           // Quando foi criada a isenção
    createdBy: z.string().nullable().optional(),           // Admin que criou
    reason: z.string().optional(),                         // Motivo (ex: "Promoção", "Bônus inicial")
  }).optional(),
