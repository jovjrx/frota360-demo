...
export default function LoginPage({ tPage }: PublicPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Criar sessão no servidor (sem Firebase)
      ...
      // Processar referral link se existir na sessão
      if (typeof window !== 'undefined') {
        const referralSlug = sessionStorage.getItem('referralSlug');
        const referrerId = sessionStorage.getItem('referrerId');
        
        if (referralSlug && referrerId) {
          try {
            await fetch('/api/driver/referral/accept', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
              body: JSON.stringify({
                referrerSlug: referralSlug,
              }),
            });
          } catch (referralErr) {
            console.warn('Erro ao processar referral:', referralErr);
            // Continua com o login mesmo se referral falhar
          } finally {
            // Limpa os dados de sessão
            sessionStorage.removeItem('referralSlug');
