...
export default async function handler(
  ...
  try {
    ...
    // ✅ Atualizar collection maestro "weekly" com status das plataformas importadas
    const batch = db.batch();
    for (const summary of summaries) {
      if (!summary.error && summary.recordsCount > 0) {
        const weeklyRef = db.collection('weekly').doc(weekId);
        const weeklyDoc = await weeklyRef.get();
        
        if (weeklyDoc.exists) {
          const weeklyData = weeklyDoc.data() as any;
          const updated = updatePlatformImported(weeklyData, summary.platform, summary.recordsCount);
          batch.update(weeklyRef, updated);
        }
      }
    }
    await batch.commit();
...
async function processAndArchiveFile({
  ...
  try {
    ...
    const aggregation = aggregatePlatformData(platform, normalizedData.rows, driverMaps);
    const now = new Date().toISOString();

    // Mapear para entries de dataWeekly (1 por identificador agregado)
    const dataWeeklyEntries: WeeklyNormalizedData[] = aggregation.entries.map((aggregated) =>
    ...
    // Substituir dados anteriores dessa semana/plataforma e gravar novos
    await deleteExistingDataWeekly(weekId, platform);
    await saveNormalizedEntries(dataWeeklyEntries);

    // Atualizar resumo
    summary.recordsCount = dataWeeklyEntries.length;
    summary.driverMatches = dataWeeklyEntries.reduce((acc, e) => (e.driverId ? acc + 1 : acc), 0);
    if (aggregation.warnings.length > 0) {
      summary.warnings.push(...aggregation.warnings);
    }

    // Marcar raw como processado
    await db.collection('rawFileArchive').doc(rawDataDocId).update({ processed: true, processedAt: now });

    console.log(`✅ ${platform.toUpperCase()}: ${dataWeeklyEntries.length} registro(s) gravados em dataWeekly e raw marcado como processado.`);
  } catch (error: any) {
...
function aggregateMyPrio(rows: any[], driverMaps: DriverMaps): AggregationResult {
  ...
  rows.forEach((row, index) => {
    ...
    // Prefer mapping by plate/"DESC CARTAO"; fallback to PRIO card number if plate is missing
    ...
    const entry = aggregates.get(key) ?? {
      referenceId:
        typeof (plateRaw || cardRaw) === 'string'
          ? String(plateRaw || cardRaw).trim()
          : key,
