...
import { getNodemailerConfig, getEmailContactInfo } from '@/lib/email-config';
...
export default async function handler(
  ...
  try {
    ...
    // Busca configurações dinâmicas do Firebase
    const smtpConfig = await getNodemailerConfig();
    const contact = await getEmailContactInfo();
    ...
    // Cria transporter com configurações dinâmicas
    const password = process.env.EMAIL_PASSWORD || process.env.EMAIL_APP_PASSWORD;
    
    if (!password) {
      console.error('❌ EMAIL_PASSWORD ou EMAIL_APP_PASSWORD não configurado');
    ...
    console.log('📧 Usando configuração de email:', { 
