...
export default function DataPage({ user, initialWeeks, initialIntegrations, tCommon, tPage, translations }: DataPageProps) {
  ...
  const handleUploadFile = useCallback(async () => {
    ...
    try {
      ...
      // Auto-process immediately so dataWeekly is created like other weeks
      toast({
        title: t('weeklyDataSources.messages.uploadSuccess', 'Upload realizado'),
        description: t('weeklyDataSources.messages.uploadSuccessDescription', 'Arquivo salvo com sucesso. Processando dados...'),
        status: 'success',
        duration: 3000,
        isClosable: true,
      });
      ...
      try {
        if (selectedWeekId) {
          await handleProcessWeek(selectedWeekId);
        }
      } catch (e) {
        console.error('Erro ao processar automaticamente após upload:', e);
      }
