...
import { getReferralConfig } from '@/lib/referral/referral-config';
...
export type BonusStatus = 'PENDING' | 'PAYABLE' | 'PAID';

export interface AffiliateBonusRecord {
  indicatorId: string;
  weekId: string;
  total: number;
  details: BonusDetail[];
...
/**
 * Conta quantos pagamentos processados (paid) o motorista teve
 * Baseado em registros com paymentStatus = 'paid' em driverPayments
 */
async function getDriverProcessedPayments(driverId: string): Promise<number> {
  const snap = await adminDb
...
/**
 * Calcula todos os bônus de indicação para uma semana
 * Todos os bônus são salvos como PENDING e precisam ser liberados via releasePendingBonuses
 */
export async function computeAllAffiliateBonuses(weekId: string): Promise<{ processed: number; indicators: number }> {
  const [referralCfg, financialCfg] = await Promise.all([
    getReferralConfig(),
  ...
  for (const driverId of driverIds) {
    ...
    baseByDriver[driverId] = referralCfg.base === 'repasse' ? repasseBase : ganhosMenosIVA;
  }
  ...
  for (const driverId of driverIds) {
    ...
    let level = 1;
    while (current && level <= referralCfg.maxLevels) {
      const pct = referralCfg.levels[level] || 0;
      if (pct > 0) {
        // eligibility: indicator must meet threshold in the same week
        const indicatorBase = baseByDriver[current] ?? (referralCfg.base === 'repasse' ? 0 : (ganhosMenosIVAByDriver[current] || 0));
        if (indicatorBase >= referralCfg.minWeeklyRevenueForEligibility) {
...
/**
 * Libera bônus PENDING para PAYABLE se motorista indicado atingiu mínimo de semanas
 * Executada semanalmente ou sob demanda
 */
export async function releasePendingBonuses(): Promise<{ released: number }> {
  const referralCfg = await getReferralConfig();
  const minWeeks = referralCfg.minWeeksToPayBonus || 4;
  ...
  for (const doc of snap.docs) {
    const data = doc.data() as AffiliateBonusRecord;
    
    // Para cada indicado, verificar se atingiu mínimo de pagamentos processados
    for (const detail of data.details) {
      const processedPayments = await getDriverProcessedPayments(detail.referredDriverId);
      if (processedPayments >= minWeeks) {
        // Libera para PAYABLE
        const docRef = adminDb.collection('affiliate_bonuses').doc(doc.id);
        batch.set(docRef, {
          status: 'PAYABLE',
          effectiveWeekId: data.weekId,
          effectiveDate: now,
          updatedAt: now,
        }, { merge: true });
        released += 1;
        break; // um bônus pode ter múltiplos detalhes, marcar uma vez
      }
    }
  }
...
/**
 * Busca bônus PAYABLE para uma semana específica
 * Marca como PAID após injetar no repasse
 */
export async function getBonusForDriverWeek(indicatorId: string, weekId: string): Promise<BonusResult | null> {
  const doc = await adminDb
    .collection('affiliate_bonuses')
    .doc(`${indicatorId}_${weekId}`)
    .get();
  ...
  // Apenas retorna se está PAYABLE ou PAID
  if (data.status !== 'PAYABLE' && data.status !== 'PAID') {
...
/**
 * Marca um bônus como PAID após ser injetado no repasse
 */
export async function markBonusAsPaid(indicatorId: string, weekId: string): Promise<void> {
