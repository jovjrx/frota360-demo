...
export class BoltClient extends BaseIntegrationClient {
  ...
  /**
   * Resolve o valor de ganhos LÍQUIDOS da viagem (net, não bruto)
   * Ordem de prioridade:
   * 1. price.net (ganhos líquidos) - PRIMARY
   * 2. trip.net_amount, trip.net, trip.earnings (fallback)
   * 3. price.amount, price.total, price.value (último recurso)
   * 
   * IMPORTANTE: Sempre retorna ganhos LÍQUIDOS, descontadas as despesas/comissões Bolt
   */
  private resolveEarnings(trip: BoltTripPayload): number {
    // Prioridade 1: price.net (ganhos líquidos - o campo correto)
    const price = trip.price || trip.total_price || trip.price_total || trip.fare;

    if (price && typeof price === 'object') {
      // PRIMEIRO: net (líquido) é o que nos importa
      if (typeof price.net === 'number' && price.net >= 0) {
        return price.net;
      }
    }

    // Prioridade 2: Campos diretos de net amount
    if (typeof trip.net_amount === 'number' && trip.net_amount >= 0) {
      return trip.net_amount;
    }
    if (typeof trip.net === 'number' && trip.net >= 0) {
      return trip.net;
    }

    // Prioridade 3: campo earnings pode conter net ou gross, use com cuidado
    if (typeof trip.earnings === 'number' && trip.earnings >= 0) {
      return trip.earnings;
    }

    // Fallback: outros campos de preço (podem ser brutos)
    ...
    if (price && typeof price === 'object') {
      ...
      if (typeof price.total === 'number' && price.total >= 0) {
        return price.total;
      }
      if (typeof price.value === 'number' && price.value >= 0) {
        return price.value;
      }
      // ÚLTIMO RECURSO: gross amount (bruto) - use só se nada mais tiver
      if (typeof price.gross === 'number' && price.gross >= 0) {
        return price.gross;
      }
    }
