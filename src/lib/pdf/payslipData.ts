...
export function buildPayslipDataFromRecord(
  ...
  const combustivelIsFixed = isFiniteNumber(record.combustivel) && record.combustivel > 0 && combustivelValue !== sumPlatform(platformData, 'myprio');
  const viaverdeIsFixed = isFiniteNumber(record.viaverde) && record.viaverde > 0 && viaverdeValue !== sumPlatform(platformData, 'viaverde');
  const hasFixedExpenses = combustivelIsFixed || viaverdeIsFixed;
  
  // Não calcular 7% se houver despesas fixas
  const despesasAdm = hasFixedExpenses ? 0 : (pickNumber(record.despesasAdm, ganhosMenosIVA * 0.07) ?? ganhosMenosIVA * 0.07);
