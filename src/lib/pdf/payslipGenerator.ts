...
export async function generatePayslipPDF(data: PayslipData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      try {
        const hasCustomLogo = !!(data as any)?.logoBuffer;
        if (hasCustomLogo) {
          // Preferir logo fornecido pelo chamador (site settings)
          doc.image((data as any).logoBuffer, 222.5, currentY, { width: 150 });
        } else {
          // Fallback para logo padrão do projeto
      ...
      doc.fontSize(16).fillColor("#000000").font("Helvetica-Bold")
        .text("RECIBO SEMANAL", leftMargin, currentY, { width: contentWidth, align: "center" });
      currentY += 28;
