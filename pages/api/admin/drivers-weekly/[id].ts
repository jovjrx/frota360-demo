...
async function handleGet(id: string, res: NextApiResponse) {
  try {
    ...
    console.error('Error fetching driver weekly record:', error);
...
async function handlePut(id: string, req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    await db.collection('driver_weekly_records').doc(id).update(updateData);
...
async function handleDelete(id: string, res: NextApiResponse) {
  try {
    await db.collection('driver_weekly_records').doc(id).delete();
    ...
    console.error('Error deleting driver weekly record:', error);
    return res.status(500).json({ error: error.message });
  }
}
