...
function DashboardContractsPageContent({ initialContract, translations }: DashboardContractsPageProps) {
  ...
  const tDashboard = createSafeTranslator(translations?.dashboard);
  const tPage = createSafeTranslator(translations?.page);

  return (
    <DashboardLayout
      translations={translations}
      title={tPage('contracts.title', 'Meus contratos')}
      subtitle={tPage('contracts.subtitle', 'Baixe, assine e envie o contrato obrigatório para continuar ativo.')}
      breadcrumbs={[{ label: tDashboard('menu.contracts', 'Contratos') }]}
    >
      {!contract && !isValidating && (
        <Alert status="info" borderRadius="md">
          <AlertIcon />
          <Text fontSize="sm">{tPage('contracts.empty', 'Ainda não há um contrato associado à sua conta. Aguarde contacto da equipa.')}</Text>
        </Alert>
      )}
      ...
      {contract?.status === 'submitted' && (
        <Alert status="info" borderRadius="md">
          <AlertIcon />
          <Text fontSize="sm">{tPage('contracts.waitingReview', 'Documento recebido. Notificaremos por email quando for aprovado.')}</Text>
        </Alert>
      )}
